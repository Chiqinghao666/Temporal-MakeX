...
static PyObject *GetEdgeTimestamp(PyObject *self, PyObject *args) {
  long ptr_val;
  long long edge_id_ll;
  if (!PyArg_ParseTuple(args, "lL", &ptr_val, &edge_id_ll)) {
    printf("Input error!\n");
    Py_INCREF(Py_None);
    return Py_None;
  }
  DataGraphWithInformation *data_graph_ptr =
      (DataGraphWithInformation *)(ptr_val);
  auto edge_ptr =
      data_graph_ptr->data_graph().FindEdge(edge_id_ll);
  if (edge_ptr == nullptr || !edge_ptr->has_timestamp()) {
    Py_INCREF(Py_None);
    return Py_None;
  }
  return PyLong_FromLongLong(edge_ptr->timestamp());
}

static PyObject *GetTemporalNeighbors(PyObject *self, PyObject *args) {
  long ptr_val;
  long long vertex_id_ll;
  long long ts_min_ll;
  long long ts_max_ll;
  int inclusive = 1;
  if (!PyArg_ParseTuple(args, "lLLLp", &ptr_val, &vertex_id_ll, &ts_min_ll,
                        &ts_max_ll, &inclusive)) {
    printf("Input error!\n");
    Py_INCREF(Py_None);
    return Py_None;
  ...
  DataGraphWithInformation *data_graph_ptr =
      (DataGraphWithInformation *)(ptr_val);
  auto neighbors = data_graph_ptr->Neighbors(
      static_cast<DataGraphVertexIDType>(vertex_id_ll),
      static_cast<DataGraph::EdgeTimestampType>(ts_min_ll),
      static_cast<DataGraph::EdgeTimestampType>(ts_max_ll), inclusive != 0);

  PyObject *py_neighbor_list = PyList_New(0);
  for (const auto &vertex_ptr : neighbors) {
    PyList_Append(py_neighbor_list, PyLong_FromLong(vertex_ptr->id()));
  }
  return py_neighbor_list;
}
...
static PyMethodDef PyExtMethods[] = {
    ...
    {"GetEdgeTimestamp", GetEdgeTimestamp, METH_VARARGS, "GetEdgeTimestamp"},
    {"GetTemporalNeighbors", GetTemporalNeighbors, METH_VARARGS,
