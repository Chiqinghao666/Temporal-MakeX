...
class DataGraphWithInformation {
 public:
  ...
  using EdgeTimestampType = typename DataGraph::EdgeType::TimestampType;
  ...
  using DataGraphMutableVertexPtr =
      typename GUNDAM::VertexHandle<DataGraph>::type;
  using DataGraphEdgePtr = typename GUNDAM::EdgeHandle<DataGraph>::type;
  using DataGraphEdgeConstPtr =
  ...
  std::vector<DataGraphVertexPtr> Neighbors(
      VertexIDType vertex_id, EdgeTimestampType ts_min,
      EdgeTimestampType ts_max, bool inclusive = true) const {
    DataGraphVertexPtr vertex_ptr =
        this->data_graph_.FindVertex(vertex_id);
    if (!vertex_ptr) {
      return {};
    }
    return Neighbors(vertex_ptr, ts_min, ts_max, inclusive);
  }

  std::vector<DataGraphVertexPtr> Neighbors(
      DataGraphVertexPtr vertex_ptr, EdgeTimestampType ts_min,
      EdgeTimestampType ts_max, bool inclusive = true) const {
    std::vector<DataGraphVertexPtr> neighbors;
    if (!vertex_ptr) {
      return neighbors;
    ...
    auto temporal_edges =
        OutEdgesWithinTime(vertex_ptr, ts_min, ts_max, inclusive);
    neighbors.reserve(temporal_edges.size());
    for (const auto& edge_ptr : temporal_edges) {
      neighbors.emplace_back(edge_ptr->dst_handle());
    }
    return neighbors;
  }

  std::vector<DataGraphEdgeConstPtr> OutEdgesWithinTime(
      DataGraphVertexPtr vertex_ptr, EdgeTimestampType ts_min,
      EdgeTimestampType ts_max, bool inclusive = true) const {
    std::vector<DataGraphEdgeConstPtr> edges;
    if (!vertex_ptr) {
      return edges;
    }
    if (ts_min > ts_max) {
      std::swap(ts_min, ts_max);
    }
    for (auto edge_it = vertex_ptr->OutEdgeBegin(); !edge_it.IsDone();
         ++edge_it) {
      if (!edge_it->has_timestamp()) {
        continue;
      }
      auto ts = edge_it->timestamp();
      bool in_range = inclusive ? (ts >= ts_min && ts <= ts_max)
                                : (ts > ts_min && ts < ts_max);
      if (!in_range) {
        continue;
      }
      edges.emplace_back(edge_it.operator->());
    }
    return edges;
  }
