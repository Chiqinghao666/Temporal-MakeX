...
inline std::string NormalizeAttrName(const std::string& raw_name) {
  std::string normalized;
  normalized.reserve(raw_name.size());
  for (const auto& ch : raw_name) {
...
inline std::string AttributeKeyToString(const AttributeKeyType& key) {
  std::stringstream ss;
  ss << key;
  return ss.str();
}

inline bool IsTimestampAttr(const std::string& attr_name) {
  static const std::vector<std::string> kTimestampNames = {
      "timestamp", "ts", "time", "event_time"};
  auto normalized = NormalizeAttrName(attr_name);
...
template <typename AttributeKeyType>
inline std::pair<int, enum BasicDataType> LocateTimestampColumn(
    const std::vector<std::pair<AttributeKeyType, enum BasicDataType>>&
        attr_info,
    size_t begin_idx) {
  for (size_t idx = begin_idx; idx < attr_info.size(); ++idx) {
    if (IsTimestampAttr(AttributeKeyToString(attr_info[idx].first))) {
      return {static_cast<int>(idx), attr_info[idx].second};
    }
  }
  return {-1, BasicDataType::kTypeUnknown};
}

template <typename TimestampType>
inline bool TryParseTimestampCell(rapidcsv::Document& doc, size_t column,
                                  size_t row, enum BasicDataType value_type,
                                  TimestampType& timestamp) {
  try {
    switch (value_type) {
      case BasicDataType::kTypeInt: {
        auto cell = doc.GetCellNew<int>(column, row);
        if (cell.second) return false;
        timestamp = static_cast<TimestampType>(cell.first);
        return true;
      }
      case BasicDataType::kTypeInt64: {
        auto cell = doc.GetCellNew<int64_t>(column, row);
        if (cell.second) return false;
        timestamp = static_cast<TimestampType>(cell.first);
        return true;
      }
      case BasicDataType::kTypeFloat: {
        auto cell = doc.GetCellNew<float>(column, row);
        if (cell.second) return false;
        timestamp = static_cast<TimestampType>(cell.first);
        return true;
      }
      case BasicDataType::kTypeDouble: {
        auto cell = doc.GetCellNew<double>(column, row);
        if (cell.second) return false;
        timestamp = static_cast<TimestampType>(cell.first);
        return true;
      }
      case BasicDataType::kTypeString: {
        auto cell = doc.GetCellNew<std::string>(column, row);
        if (cell.second || cell.first.empty()) return false;
        timestamp = static_cast<TimestampType>(std::stoll(cell.first));
        return true;
      }
      case BasicDataType::kTypeDateTime: {
        auto cell = doc.GetCellNew<std::string>(column, row);
        if (cell.second || cell.first.empty()) return false;
        DateTime dt(cell.first.c_str());
        timestamp = static_cast<TimestampType>(dt.unix_time());
        return true;
      }
...
int ReadCSVEdgeSetFileWithCallback(
  ...
  try {
    ...
    auto [timestamp_col_idx, timestamp_type] =
        LocateTimestampColumn(attr_info, 5);
    ...
    for (size_t row = 0; row < sz; row++) {
      ...
      if (timestamp_col_idx >= 0) {
        typename GraphType::EdgeType::TimestampType timestamp_value = 0;
        if (TryParseTimestampCell(edge_set_file, timestamp_col_idx, row,
                                  timestamp_type, timestamp_value)) {
          edge_handle->set_timestamp(timestamp_value);
        }
      }
...
int ReadCSVEdgeFileWithCallback(const std::string& e_file, GraphType& graph,
  ...
  try {
    ...
    auto [timestamp_col_idx, timestamp_type] =
        LocateTimestampColumn(attr_info, 4);
    ...
    for (size_t row = 0; row < sz; row++) {
      ...
      if (timestamp_col_idx >= 0 && r) {
        typename GraphType::EdgeType::TimestampType timestamp_value = 0;
        if (TryParseTimestampCell(edge_file, timestamp_col_idx, row,
                                  timestamp_type, timestamp_value)) {
          edge_handle->set_timestamp(timestamp_value);
        }
      }
