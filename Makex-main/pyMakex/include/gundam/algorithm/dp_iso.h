...
constexpr size_t large_query_edge = 3;
constexpr size_t adj_vertex_limit = 20000000;
...
template <enum MatchSemantics match_semantics, typename QueryGraph,
          typename TargetGraph, 
          class DataVertexIDType = typename TargetGraph::VertexType::IDType, 
          class PatternVertexPtr = typename GUNDAM::VertexHandle<const QueryGraph>::type,
          class FilterCallback = nullptr_t>
inline bool InitCandidateSet_U_V(
    QueryGraph &query_graph, TargetGraph &target_graph,
    const DataVertexIDType x_id_,
    const DataVertexIDType y_id_,
    const PatternVertexPtr &pivot_x_ptr,
    const PatternVertexPtr &pivot_y_ptr,
  ...
  for (auto query_vertex_iter = query_graph.VertexBegin();
    ...
    TargetVertexHandle target_vertex_handle_pivot;
    if(query_vertex_handle->id() == pivot_x_ptr->id()){ 
      
      target_vertex_handle_pivot = target_graph.FindVertex(x_id);
      if (!filter_callback(query_vertex_handle, target_vertex_handle_pivot)) {
          return false;
    }
      if (!NeighborLabelFrequencyFiltering<match_semantics, QueryGraph,
                                              TargetGraph>(
                  query_vertex_handle, target_vertex_handle_pivot)) {
            return false;
      }

      query_vertex_candidate.emplace_back(target_vertex_handle_pivot);
    ...
    if (query_vertex_handle->id() == pivot_y_ptr->id()) {

      target_vertex_handle_pivot = target_graph.FindVertex(y_id);
    ...
    if(query_vertex_handle->id() != pivot_x_ptr->id() && query_vertex_handle->id() != pivot_y_ptr->id()){
      if constexpr (TargetGraph::graph_has_vertex_label_index) {
        QueryVertexHandle farther_node_ptr;
        QueryEdgeLabelType farther_edge_label;
        for (auto edge_it = query_vertex_handle->InEdgeBegin(); !edge_it.IsDone();
          edge_it++) {
            farther_node_ptr = edge_it->src_handle();
            farther_edge_label = edge_it->label();
        }

        std::vector<TargetVertexHandle> farther_target_node_ptr = candidate_set.find(farther_node_ptr)->second;
        std::unordered_set<TargetVertexHandle> candidate_node_current;

        for (auto &vertex_handle_ : farther_target_node_ptr) {
            auto out_vertex_iter = vertex_handle_->OutVertexBegin(farther_edge_label);
            if (out_vertex_iter.IsDone()) {
                continue;
            }

            for (; !out_vertex_iter.IsDone(); ++out_vertex_iter) {
                TargetVertexHandle target_vertex_handle = out_vertex_iter;
                candidate_node_current.insert(target_vertex_handle);
            }
        }
        


        for(auto& target_vertex_iter : candidate_node_current) {
...
(edge_state == EdgeState::kIn) ? edge_it->src_handle(): edge_it->dst_handle();        auto [visited_target_handle_set_it, visited_target_handle_set_ret] =
...
inline void TopuSort(QueryGraph &query_graph, InDegreeContainer &in_degree,
  ...
  while (!topu_sort_queue.empty()) {
    ...
    std::vector<typename QueryGraph::EdgeType> out_edges;
    for (auto edge_it = query_vertex_handle->OutEdgeBegin(); !edge_it.IsDone(); edge_it++) {
      out_edges.push_back(*edge_it);
    }
    std::sort(out_edges.begin(), out_edges.end(), [](const auto& a, const auto& b) {
      return a.id() < b.id();
    });

    for (auto& edge : out_edges) {
      if (used_edge.count(edge.id())) continue;
      QueryVertexHandle next_vertex_handle = edge.dst_handle();
      in_degree[next_vertex_handle]--;
      used_edge.insert(edge.id());
      if (in_degree[next_vertex_handle] == 0) {
        topu_sort_queue.push(next_vertex_handle);
      }
    }

    std::vector<typename QueryGraph::EdgeType> in_edges;
    for (auto edge_it = query_vertex_handle->InEdgeBegin(); !edge_it.IsDone(); edge_it++) {
      in_edges.push_back(*edge_it);
    }
    std::sort(in_edges.begin(), in_edges.end(), [](const auto& a, const auto& b) {
      return a.id() < b.id();
    });

    for (auto& edge : in_edges) {
      if (used_edge.count(edge.id())) continue;
      QueryVertexHandle next_vertex_handle = edge.src_handle();
      in_degree[next_vertex_handle]--;
      used_edge.insert(edge.id());
      if (in_degree[next_vertex_handle] == 0) {
        topu_sort_queue.push(next_vertex_handle);
...
inline void DFSPaths(QueryGraph &query_graph, typename VertexHandle<QueryGraph>::type current_vertex, std::vector<std::vector<typename VertexHandle<QueryGraph>::type>>& result, std::vector<typename VertexHandle<QueryGraph>::type> current_path, std::set<typename VertexHandle<QueryGraph>::type>& visited, std::map<typename VertexHandle<QueryGraph>::type, int>& in_degree) {
    using QueryVertexHandle = typename VertexHandle<QueryGraph>::type;
...
bool is_leaf = true;
    for (auto edge_it = current_vertex->OutEdgeBegin(); !edge_it.IsDone(); edge_it++) {
        QueryVertexHandle next_vertex = edge_it->dst_handle();
        if (visited.find(next_vertex) == visited.end()) {
            is_leaf = false;
            DFSPaths(query_graph, next_vertex, result, current_path, visited, in_degree);
        }
    }
    ...
    visited.erase(current_vertex);
}


template <class QueryGraph>
inline void GetAllPathsFromRootsToLeaves(QueryGraph &query_graph, std::vector<std::vector<typename VertexHandle<QueryGraph>::type>>& result) {
    using QueryVertexHandle = typename VertexHandle<QueryGraph>::type;
    std::map<QueryVertexHandle, int> in_degree;

BFS(query_graph, in_degree);
    for (auto it = query_graph.VertexBegin(); !it.IsDone(); it++) {
        QueryVertexHandle start_vertex = it;
        if (in_degree[start_vertex] == 0) {
            std::vector<QueryVertexHandle> current_path;
            std::set<QueryVertexHandle> visited;
            DFSPaths(query_graph, start_vertex, result, current_path, visited, in_degree);
        }
    }
}
...
template <class QueryGraph>
inline void DFSPaths_with_edge_label(QueryGraph &query_graph, typename VertexHandle<QueryGraph>::type current_vertex, std::vector<std::vector<std::pair<typename VertexHandle<QueryGraph>::type, int>>>& result, std::vector<std::pair<typename VertexHandle<QueryGraph>::type, int>> current_path, std::set<typename VertexHandle<QueryGraph>::type>& visited, std::map<typename VertexHandle<QueryGraph>::type, int>& in_degree) {
    using QueryVertexHandle = typename VertexHandle<QueryGraph>::type;
...
bool is_leaf = true;
    for (auto edge_it = current_vertex->OutEdgeBegin(); !edge_it.IsDone(); edge_it++) {
        ...
        if (visited.find(next_vertex) == visited.end()) {
            is_leaf = false;
            current_path.push_back(std::make_pair(current_vertex, edge_it->label()));
            DFSPaths_with_edge_label(query_graph, next_vertex, result, current_path, visited, in_degree);
            current_path.pop_back();
        }
    }

    if (is_leaf) {
        current_path.push_back(std::make_pair(current_vertex, -1));
        result.push_back(current_path);
        current_path.pop_back();
    }
...
template <class QueryGraph>
inline void GetAllPathsFromRootsToLeaves_with_edge_label(QueryGraph &query_graph, std::vector<std::vector<std::pair<typename VertexHandle<QueryGraph>::type, int>>>& result) {
    using QueryVertexHandle = typename VertexHandle<QueryGraph>::type;
    std::map<QueryVertexHandle, int> in_degree;
...
BFS(query_graph, in_degree);
    for (auto it = query_graph.VertexBegin(); !it.IsDone(); it++) {
        QueryVertexHandle start_vertex = it;
        if (in_degree[start_vertex] == 0) {
            std::vector<std::pair<QueryVertexHandle, int>> current_path;
            std::set<QueryVertexHandle> visited;
            DFSPaths_with_edge_label(query_graph, start_vertex, result, current_path, visited, in_degree);
        }
    }
}
...
template <class QueryGraph>
inline void GetAllPathsFromRootsToLeaves_Variable(QueryGraph &query_graph, std::vector<std::vector<typename VertexHandle<QueryGraph>::type>>& result) {
    using QueryVertexHandle = typename VertexHandle<QueryGraph>::type;
    std::map<QueryVertexHandle, int> in_degree;
  ...
  for (auto edge_it = is_out_direction ? query_vertex_handle->OutEdgeBegin()
    ...
    if constexpr (TargetGraph::vertex_has_edge_label_index) {
      for (auto target_edge_it =
        ...
        auto it1 = std::find_if(l1.begin(), l1.end(), 
                                [temp_target_vertex_handle](const auto& candidate) {
                                  return candidate->id() == temp_target_vertex_handle->id();
                                });
        if (it1 != l1.end()) {
          find_flag = true;
          break;
      ...
      for (auto target_edge_it = is_out_direction
        ...
        TargetVertexHandle temp_target_vertex_handle = is_out_direction ? target_edge_it->dst_handle(): target_edge_it->src_handle();        
        if (!EdgeCheck<QueryGraph, TargetGraph>(
                query_vertex_handle, next_vertex_handle, target_vertex_handle,
                temp_target_vertex_handle, out_edge_count, in_edge_count)) {
          continue;
...
inline bool DAGDP(
  ...
  for (auto &vertex_handle : topu_seq) {
    ...
    for (auto &candidate_it : l) {
      bool candidate_flag_out = true;
      bool candidate_flag_in = true;
      if(out_edge_count.size() > 0){
        candidate_flag_out = _DAGDP::CheckIsLegal<true, QueryGraph, TargetGraph>(
          vertex_handle, candidate_it, used_vertex, out_edge_count,
          in_edge_count, candidate_set);
      }
      if(in_edge_count.size() > 0){
        candidate_flag_in = _DAGDP::CheckIsLegal<false, QueryGraph, TargetGraph>(
          vertex_handle, candidate_it, used_vertex, out_edge_count,
          in_edge_count, candidate_set);
      }
      if(candidate_flag_out && candidate_flag_in){
      ...
      [&result_count, &user_callback, &user_callback_lock,
       &user_callback_has_return_false](auto &match_state) {
        ...
        if (!user_callback_has_return_false) {
          if constexpr (!std::is_null_pointer_v<decltype(user_callback)>) {
...
inline int DPISO_Recursive(QueryGraph &query_graph, TargetGraph &target_graph,
  ...
  if (!RefineCandidateSet(query_graph, target_graph, candidate_set)) {
    std::cout<<"refine end!"<<std::endl;	  
    return 0;
  }
  auto t_end = clock();
  std::cout<<"refine time is"<<(1.0*t_end-t_begin)/CLOCKS_PER_SEC<<std::endl;
