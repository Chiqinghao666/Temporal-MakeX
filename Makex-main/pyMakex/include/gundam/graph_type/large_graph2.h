...
class LargeGraph2 {
 ...
 private:
  ...
  class VertexData
   ...
   public:
    ...
    bool HasOutEdge(const VertexData *vertex_ptr) const {
      auto it1 =
          out_edges_by_dst_el_.Find(const_cast<VertexData *>(vertex_ptr));
      if (it1 == out_edges_by_dst_el_.cend()) return 0;
      return 1;
  ...
  class EdgeData
   ...
   using AttributeKeyType = EdgeAttributeKeyType;
    ...
    EdgeData(const EdgeIDType &id, const EdgeLabelType &label, VertexData *src,
             VertexData *dst)
        : id_(id), label_(label), src_(src), dst_(dst), timestamp_(0),
    ...
    void set_timestamp(const TimestampType &timestamp) {
 ...
 public:
  ...
  using EdgeTimestampType = typename EdgeType::TimestampType;
